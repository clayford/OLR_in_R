...
Another way to summarize the results is with _cumulative probabilities_. For example, what's the the probability a randomly selected student can handle _medium heat or less_? Using the `cumsum()` we can calculate cumulative sums of proportions and answer this question. Below we take the proportions and "pipe" into the `cumsum()` function using the base R pipe operator. A keyboard shortcut to insert the pipe operator is *Ctrl + Shift + M* (Win/Linux) or *Cmd + Shift + M* (Mac)

```{r}
proportions(resp) |> cumsum()
```

Based on this calculation I might estimate that a randomly selected student has a probability of about 0.71 of liking _medium or lower_ spiciness. We can state this mathematically as follows (using LaTeX):
...
What about age or weight or ethnicity? Do these affect the _cumulative probability_ of spiciness preference?
...
The goal today is to learn the basics of Ordinal Logistic Regression by developing a model to predict the _cumulative probability_ of pain improvement after treatment adjusting for age and sex. 
...
```{r}
xtabs(~ Improved, data = Arthritis, addNA = TRUE)
```
...
- Investigate the Sex variable with respect to Improved
- Investigate the Age variable with respect to Improved
...
First let's interpret the Treatment coefficient: 1.568 

This on the _log odds_, or _logit_, scale. This says being in the Treated group increases the log odds of improvement by 1.568.

Reminder: log odds is probability [0,1] rescaled to range from (-Inf,Inf).

If we exponentiate the coefficient we get an _odds ratio_.

```{r}
exp(coef(m1))
```

This says the estimated odds that a Treated subject's response has _higher improvement_ is about 4.8 times higher than the odds that a Placebo subject's response has higher improvement. Recall the levels of Improved:

None < Some < Marked

There are two breakpoints:

1. None vs Some or Marked
2. None or Some vs Marked

- Expected odds that Treated subject experiences _Some or Marked_ improvement is 4.8 times the odds that a Placebo subject experiences Some or Marked improvement.
- Expected odds that Treated subject experiences _Marked_ improvement is 4.8 times the odds that a Placebo subject experiences Marked improvement.

It doesn't matter which comparison we make, the model assumes the same odds ratio. This is the _proportional odds assumption_.
...
Notice we have _two intercepts_. The separate intercepts are for _the levels below which we calculate cumulative probabilities_. Recall the levels of Improved:
...
So we have _two models_ depending on the cumulative probability we want to model:
...
Now let's use our model and compute cumulative probabilities "by hand". Before we do that, we need to know how to extract the intercepts and coefficients from the model object. The model object is a list. We use the `$` operator to extract elements from it. The intercepts are in the `zeta` element and the coefficients are in the `coefficients` element. (Can also use `coef()` to extract coefficients.)
...
That's simply the `None|Some` intercept: 0.7499107. To convert to probability, we can use the `plogis()` function, which calculates the inverse logit. Below we pipe the `None|Some` intercept into `plogis()` and save as `p1`.

```{r}
# P(None) when Treatment = "Placebo"
(p1 <- m1$zeta["None|Some"] |> plogis())
```

The log odds of None given a subject was on Treatment (Treatment = 1):

log odds(None) = 0.7499 - 1.568*1 = -0.8181. Again we convert to probability.

```{r}
# P(None) when Treatment = "Treated"
(p2 <- (m1$zeta["None|Some"] - m1$coefficients) |> plogis())
```

So we predict subjects on Placebo will see no improvement with probability 0.68 while subjects on Treatment will see no improvement with probability 0.31. 

### Prob(Some or lower)

The log odds of Some or lower given a subject was on Placebo (Treatment = 0):

log odds(Some or lower) = 1.5498756 - 1.567644*0 = 1.5498756

That's simply the `Some|Marked` intercept: 1.5498756. Use `plogis()` to convert to probability

```{r}
# P(Some or lower) when Treatment = "Placebo"
(p3 <- m1$zeta["Some|Marked"] |> plogis())
```

The log odds of Some or lower given a subject was on Treatment (Treatment = 1):

log odds(Some or lower) = 1.5498756 - 1.567644*1 = -0.0177684. Convert the log odds to probability.

```{r}
# P(Some or lower) when Treatment = "Treated"
(p4 <- (m1$zeta["Some|Marked"] - m1$coefficients) |> plogis())
```

So we predict subjects on Placebo will see None or Some improvement with probability 0.82 while subjects on Treatment will see None or Some improvement with probability 0.50. 
...
## Understanding proportional Odds
...
Notice if we do the same calculations with p3 and p4 we get the _same result_.

Predicted probability of Improved = Some or None on Treatment is 0.495558, which is stored in `p4`. The odds are calculated as 0.495558/(1 - 0.495558)

```{r}
(odds4 <- p4/(1 - p4))
```

Predicted probability of Improved = Some or None on Placebo is 0.8248958 , which is stored in `p3`. The odds are calculated as 0.8248958 /(1 - 0.8248958 )
...
The _ratio between treatment odds and placebo odds are the same_ for both P(None) and P(Some or lower). That's the definition of a proportional relationship. In other words, we have _proportional odds_.

```{r}
matrix(data = c(odds2, odds1, odds4, odds3), ncol = 2,
       dimnames = list("Treatment" = c("Treated", "Placebo"),
                       "Odds" = c("None", "Some or lower")))
```

The odds are proportional since their ratios are the same:

```{r}
c(0.441431/2.116811, 0.9823884/4.7108843)
```

Notice the interpretation is _reversed_ from what we did above! The estimated odds that a Treated subject's response has _lower_ improvement is 0.21 times (or 79% lower) lower than the odds that a Placebo subject's response has lower improvement. (1 - 0.21 = 0.79)

Why is this odds ratio (0.2085359) different from `exp(coef(m1))` (4.795338)?

```{r}
exp(coef(m1))
```

If we take the _log_ of the odds ratios we get the log odds model coefficient for Treatment with a _change in sign_. 

```{r}
c(log(odds2/odds1), log(odds4/odds3))
```

This is identical to `coef(m1)`, but with a change in sign. 

```{r}
coef(m1)
```

`polr()` multiplies the coefficients in the summary output by -1 so when you exponentiate to get the odds ratio _higher levels of predictors correspond to the response falling in the higher end of the ordinal scale_. 

None < Some < Marked

- Instead of estimating odds of None, we estimate odds of Some or Marked.
- Instead of estimating odds of None or Some, we estimate odds of Marked.
...
The substance of the result remains the same, but the interpretation is reversed: 
- Expected odds that Treated subject experiences _No_ improvement is 0.21 times (or 79% less) the odds that a Placebo subject experiences No improvement.
- Expected odds that Treated subject experiences _None or Some_ improvement is 0.21 times (or 79% less) the odds that a Placebo subject experiences None or Some improvement.

In most cases, _we prefer the former interpretation_. We want to understand how variables affect the probability of moving to higher categories. (i.e., going from None to Some/Marked, or None/Some to Marked)

## CODE ALONG 2

Model Improved as a function of Treatment and Sex and save the model object as `m2`. In other words, we want to understand the association between Treatment and Improved adjusted for Sex. Interpret the coefficient on Treatment. 
...
Proportional odds logistic regression assumes _the coefficients are independent of the category levels_. This means we assume the estimated odds ratio of Treatment, 4.795, is the same whether we're talking about comparing Some or higher versus Non, or Marked versus None or Some on the Improvement scale.
...
The authors of the {car} package warn, _"It's our experience that the proportional-odds assumption is rarely supported by a hypothesis test._ (Fox and Weisberg, 2019, p. 322)
...
Notice these are _NOT cumulative probabilities_. These are expected probabilities for each level. They sum to 1. We see that `type = "class"` setting is simply picking the category with the highest probability.

The `predict()` method unfortunately does not provide standard errors and confidence intervals for the predictions. However we can use the {emmeans} package to calculate these. Below we obtain estimated probabilities of each level of Improved conditional on Treatment. The `mode = "prob"` argument ensures predictions are of the probabilities of each class of the ordinal response.
...
Use {emmeans} to calculate estimated probabilities for each level of Improved _adjusted for Sex_ using model object `m2`.
...
If you want something created in ggplot2, the {ggeffects} package provides the `ggeffect()` function and associated plotting method. Set `connect_lines = TRUE` to connect the lines.

```{r}
ggeffect(m1, terms = "Treatment") |> plot(connect_lines = TRUE)
```

Let's fit a model with Age and its interaction to see how we can visualize the effects of a numeric predictor. 

```{r}
m3 <- polr(Improved ~ Treatment + Age + Treatment:Age, 
           data = Arthritis, Hess = TRUE)
summary(m3)
```

The `Effect()` function combined with its `plot()` method show that as age increases, the probability of Marked improvement increases. The little lines on the x-axis are the observed data. We call that a "rug".

```{r}
Effect(mod = m3, focal.predictors = "Age") |> plot()
```

To create the plot above, the Treatment predictor has to be held at a value. The only values in the data are 0 (Placebo) or 1 (Treated). However the `Effect()` function "averages" over the levels of Treatment. We can save the result of the `Effect()` function and see what values are being used to make the predictions.

```{r}
e.out <- Effect(mod = m3, focal.predictors = "Age")
# what's being plotted
e.out
```

And what's being plugged into the model to make predictions.

```{r}
e.out$model.matrix
```

The value for Treatment is just the proportion Treated.

```{r}
mean(Arthritis$Treatment == "Treated")
```

This is a "weighted average" that weights the levels of the factor in proportion to sample size.

If you wanted the plot with Treatment held at 0 or 1, probably easiest to just include Treatment as a focal predictor. In fact, since we fit an interaction, we ought to include Treatment since our model says the effect of Age depends on Treatment and vice versa.

```{r}
Effect(mod = m3, focal.predictors = c("Age", "Treatment")) |> 
  plot()
```


Another visualization option for numeric predictors is the "stacked" plot, though there is no indication of uncertainty. The height of each region corresponds to expected probability. The probability of "Some" improvement is about the same throughout the age range. The probability of "Marked" improvement increases as age increase. 

```{r}
Effect(m3, focal.predictors = "Age") |> 
  plot(axes=list(y=list(style="stacked")))
```

Again, we can include Treatment.

```{r}
Effect(m3, focal.predictors = c("Age", "Treatment")) |> 
  plot(axes=list(y=list(style="stacked")))
```


As before, a {ggplot2} version is available with the `ggeffects()` function.

```{r}
ggeffect(m3, terms = "Age") |> plot()
```

Including the interaction.

```{r}
ggeffect(m3, terms = c("Age", "Treatment")) |> plot()
```


## CODE ALONG 6

Let's work through a slightly more sophisticated example with three predictors.
...
- Visualize the model
- Is this model "better" than `m2`? Use `anova(m2, m4)` to test.
...
Let's calculate the proportion of each response within the first simulation using `xtabs()` and `proportions()`. The last function, `as.data.frame()` converts the table to a data frame and names the column containing the proportions "P" (the default is "Freq").
