...
A good model should generate data that looks similar to our observed data. We can use the `simulate()` function to rapidly simulate many new data sets using our model.

```{r}
sims <- simulate(m1, nsim = 250)
```

One quick assessment to simply find the mean proportion of None, Some and Marked for the simulated data sets.
...
```{r}
sims_df <- lapply(sims, p_df) |> do.call(what = "rbind",args = _)
```
