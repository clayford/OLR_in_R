...
This on the log odds scale. If we exponentiate we get an _odds ratio_.
...
Let's fit a model with a numeric predictor, Age, to see how we can visualize its effects.

```{r}
m3 <- polr(Improved ~ Treatment + Age, data = Arthritis, Hess = TRUE)
summary(m3)
```

The `Effect()` function combined with its `plot()` method show that as age increases, the probability of Marked improvement increases.
...
Again, a {ggplot2} version is available with the `ggeffects()` function.

```{r}
ggeffect(m3, terms = "Age") |> plot()
```

If we want, we can save the output of `ggeffects()` and create our own plot. For example, we may want to combine the three plots above into one. Notice we need to set the "response.level" column as an ordered factor.

```{r}
eDF <- ggeffect(m3, terms = "Age")
eDF$response.level <- factor(eDF$response.level, 
...
ggplot(eDF) +
  ...
  theme_classic() +
  labs(fill = "Improvement", color = "Improvement", 
       x = "Age", y = "Probability")
